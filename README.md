...
该开关会尽量使用 deterministic runtime guard，并在 CUDA 下关闭 `cudnn_benchmark` 与 TF32。稳定模式同 seed 复现实验应优先使用仓库脚本：

```powershell
.\scripts\launch_formal_train_stable.ps1 -RunName stable_seed0_a
.\scripts\launch_formal_train_stable.ps1 -RunName stable_seed0_b
```

该脚本会在启动 Python 前设置 `PYTHONHASHSEED=0` 和 `CUBLAS_WORKSPACE_CONFIG=:4096:8`，并传入 `--strict-reproducibility`、`--no-deterministic-warn-only`、`--no-enable-tf32`、`--no-enable-cudnn-benchmark` 以及 AMP / compile / channels_last 关闭参数。A/B 复现实验只应改变 `-RunName`，其他参数保持一致。运行后可用以下命令检查 contract：
