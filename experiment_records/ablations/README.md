...
outputs run_name、`experiment_records` 目录和 checkpoint_store 文件名应使用同一个 slug。`experiment_records` 不保存 checkpoint 本体；checkpoint 本体保存在本地 `checkpoint_store/`，该目录被 Git 忽略，不应提交 `.pt`、`.pth` 或 `.ckpt` 文件。本目录只提交 curated logs，`run_record.md` 会记录 checkpoint_store 路径和复制状态。
