...
- `ablations/` 下各目录对应 F/R 消融实验记录，目录 slug 为 `<short_id>_ablation_<canonical_id>`，例如 `F1_ablation_no_frontier_channel` 和 `R5_ablation_no_efficiency_penalties`。
- `templates/` 保存人工记录和 artifact 检查模板。

outputs run_name、`experiment_records` 目录和 checkpoint_store 推荐文件名应保持同一 slug。checkpoint_store 被 Git 忽略，不应提交模型权重或 checkpoint；`experiment_records` 只提交 curated logs。
