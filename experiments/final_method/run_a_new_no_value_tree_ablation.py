...
from env.value_state_builder import VALUE_REPLACEMENT_STRATEGY_ZERO_VALUE_STATE
...
DEFAULT_EXPERIMENT_ID = "Anew_D"
DEFAULT_METHOD_ID = "Anew_D_no_value_tree"
DEFAULT_METHOD_NAME = "no_value_tree"
DEFAULT_ABLATION_GROUP = "structural"
DEFAULT_ABLATION_ID = "D"
DEFAULT_ABLATION_NAME = "no_value_tree"
...
def build_train_args(
    *,
    ...
    train_args.extend(["--value-replacement-strategy", VALUE_REPLACEMENT_STRATEGY_ZERO_VALUE_STATE])
    train_args.append("--no-value-tree")
    return train_args
...
def _try_append_artifact_index(run_dir: Path, manifest_path: Path) -> None:
    ...
    if not artifact_index_path.exists():
        print(f"[A_new_D] warning: artifact_index.json not found at {artifact_index_path}")
        return
    try:
            ...
            payload["final_method_artifacts"].append(
                {
                    "path": rel_path,
                    "required": True,
                    "category": "a_new_no_value_tree_ablation_manifest",
                }
            )
        else:
        ...
        print(f"[A_new_D] warning: failed to append no-value-tree manifest to artifact_index: {exc}")
...
def manifest_payload(
    *,
    ...
    channel_count = advantage_canvas_channel_count_for_schema(cfg.advantage_canvas_schema)
    return {
        "schema_version": "a_new_no_value_tree_ablation_manifest/v1",
        "method_family": "A_new_structural_ablation",
        "baseline_method": "A_new_final_4ch_no_frontier_raster",
        "experiment_id": cfg.experiment_id,
        "method_id": cfg.method_id,
        ...
        "runner_entrypoint": runner_entrypoint,
        "notes": [
            "Anew_D_no_value_tree is aligned to the current A_new four-channel advantage canvas.",
            "The advantage branch keeps free, obstacle, visit_count_log_norm, and recent_trajectory_decay.",
            "The value-tree branch receives zero-value-state tensors with all masks false.",
            "No legacy 5-channel frontier raster or legacy D artifact is restored.",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="A_new D no-value-tree structural ablation launcher")
    ...
    if args.method_id != DEFAULT_METHOD_ID:
        raise ValueError(f"Anew_D_no_value_tree method_id must be {DEFAULT_METHOD_ID!r}.")
    if args.method_name != DEFAULT_METHOD_NAME:
        raise ValueError(f"Anew_D_no_value_tree method_name must be {DEFAULT_METHOD_NAME!r}.")
    ...
    print(f"[A_new_D] command: {_command_text(command)}", flush=True)
    ...
    print("[A_new_D] runtime_contract:")
    print(json.dumps(
        manifest_payload(
            cfg=cfg,
            runner_entrypoint=runner_entrypoint,
    ...
    run_dir = train_q_agent.run_training(cfg, run_mode=f"a_new_no_value_tree_ablation_{args.run_stage}")
